...
async function setWithRetry(service, account, value) {
  try {
    ...
    try {
      ...
      const err = new Error(`keytar setPassword failed: ${e2 && e2.message ? e2.message : String(e2)} | meta=${JSON.stringify(meta)}`)
      err.cause = e2
      throw err
    }
  }
...
module.exports = { SERVICE_PREFIX, serviceNameFor, storeToken, getToken, deleteToken }
