...
const migrateUserTokensForProviders = async (
	user,
	providers = ['spotify', 'discord', 'twitch'],
	db = defaultDb
) => {
		...
		try {
			...
			if (!existing) {
				...
				if (fallback) {
					...
					if (existing)
						console.log(
							`Found fallback keystore entry for ${provider} and copied to user ${user._id}`
						)
				}
			}
			...
			if (existing) {
				try {
					...
					console.log(
						`Keystore already had ${provider} for ${user._id}; marker set in DB`
					)
					migrated.push(provider)
				} catch (e) {
					console.error(
						`Failed to set marker for ${provider} for user ${user._id}:`,
						e
					)
			...
			if (provider === 'spotify') {
				if (
					user.spotify &&
					(user.spotify.refresh_token || user.spotify.access_token)
				) {
					blob = user.spotify
				} else if (
			...
			if (provider === 'discord') {
				if (
					user.discord &&
					(user.discord.refresh_token || user.discord.access_token)
				) {
					blob = user.discord
			...
			if (provider === 'twitch') {
				if (
					user.twitch &&
					(user.twitch.refresh_token || user.twitch.access_token)
				) {
					blob = user.twitch
			...
			console.error(
				`Failed to migrate ${provider} for user ${user && user._id}:`,
				e
			)
			errors.push({ provider, error: String(e) })
		}
...
const migrateAllUsers = async (
	options = { compact: true, removeLegacy: false },
	db = defaultDb
) => {
	// Try to surface which physical DB file is being used so callers can verify where changes land
	try {
		const dbFile =
			db && db.users && db.users.filename ? db.users.filename : 'unknown'
	...
	return new Promise((resolve) => {
		db.users.find({}, async (err, users) => {
			...
			for (const user of users) {
				...
				try {
					const result = await migrateUserTokensForProviders(
						user,
						undefined,
						db
					)
						...
						for (const p of result.migrated) {
							...
							if (options.removeLegacy) {
								try {
									...
									console.error(
										'Failed to remove legacy fields for',
										user._id,
										p,
										e
									)
			...
			if (
				...
				db &&
				db.users &&
				db.users.persistence &&
				typeof db.users.persistence.compactDatafile === 'function'
			) {
				try {
...
const anyUsersHaveLegacyTokens = async (db = defaultDb) => {
	return new Promise((resolve) => {
		db.users.find({}, (err, users) => {
			...
			for (const user of users) {
				...
				if (
					...
					return resolve(true)
				if (user.appAuthorizationCode || user.authorizationCode)
					return resolve(true)
			}
			resolve(false)
		})
...
module.exports = {
	migrateAllUsers,
	migrateUserTokensForProviders,
	anyUsersHaveLegacyTokens,
}
